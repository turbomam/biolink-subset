...
all: site
site: gen-project gendoc project/json/biolink_subset.json
%.yaml: gen-project
...
# MAM 2023-05-07 added yaml extension because dir now contains valid and invalid subdirs
#   todo: wouldn't include any examples in other serializations like TTL, JSON etc.
# additionally changed/limited to valid Collection examples
gen-examples:
	cp src/data/examples/valid/entityCollection*.yaml $(EXAMPLEDIR)
...
.PHONY: delete-examples-outputs

delete-examples-outputs:
	rm -rf $(EXAMPLEDIR)/output

test-examples: delete-examples-outputs examples/output

examples/output: src/biolink_subset/schema/biolink_subset.yaml
	mkdir -p $@
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/biolink_subset/schema/*yaml src/*/datamodel/*py src/data src/docs tests src/*/_version.py project/reports/ src/local_schemasheets/ data_harmonizer/
...
target/usage_template.tsv: src/biolink_subset/schema/biolink_subset.yaml
	mkdir -p $(dir $@)
...
project/json/biolink_subset.json: src/biolink_subset/schema/biolink_subset.yaml
	mkdir -p $(@D)
	$(RUN) gen-linkml \
		--format json  \
