...
JSON_SCHEMA_FILE := project/jsonschema/biolink_subset.schema.json
...
run-linkml-validation: src/biolink_subset/schema/biolink_subset.yaml \
src/data/examples/invalid/entityCollection-undefined-slot.yaml
	# entityCollection is assumed as the target-class because it has been defined as the tree_root in the schema
	- $(RUN) linkml-validate \
	  --schema $^


src/data/dh_vs_linkml_json/entityCollection_linkml_raw.yaml: src/data/dh_vs_linkml_json/entity_dh.json
	$(RUN) dh-json2linkml \
		--input-file $< \
		--output-file $@ \
		--output-format yaml \
...
src/data/dh_vs_linkml_json/entityCollection_linkml_normalized.yaml: src/data/dh_vs_linkml_json/entityCollection_linkml_raw.yaml
	$(RUN) linkml-normalize \
		--schema src/biolink_subset/schema/biolink_subset.yaml \
		--output $@ \
		--no-expand-all $<

src/data/dh_vs_linkml_json/entries.json: src/data/dh_vs_linkml_json/entityCollection_linkml_normalized.yaml
	$(RUN) linkml-json2dh \
		--input-file $< \
		--input-format yaml \
		--output-dir $(dir $@)
